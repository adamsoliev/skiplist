...
static void BM_Insert(benchmark::State& state) {
  ...
  for (auto _ : state) {
    ...
    for (size_t i = 0; i < num_items; ++i) {
      list.insert(data.key(i).data(), data.key(i).size(),
                  data.value(i).data(), data.value(i).size());
    }
...
static void BM_InsertRandom(benchmark::State& state) {
  ...
  for (auto _ : state) {
    ...
    for (size_t i = 0; i < num_items; ++i) {
      ...
      list.insert(data.key(idx).data(), data.key(idx).size(),
                  data.value(idx).data(), data.value(idx).size());
    }
...
static void BM_MixedReadWrite(benchmark::State& state) {
  ...
  state.SetItemsProcessed(
      static_cast<int64_t>(state.iterations() * num_threads * ops_per_thread));
  state.SetLabel(SkipListType::kName);
}
...
// Sequential Insert
BENCHMARK(BM_Insert<MinilsmWrapper>)
    ->Name("Minilsm/Insert")
    ->Range(1000, 100000)
    ->Unit(benchmark::kMillisecond);
BENCHMARK(BM_Insert<RocksDBWrapper>)
    ->Name("RocksDB/Insert")
    ->Range(1000, 100000)
    ->Unit(benchmark::kMillisecond);
BENCHMARK(BM_Insert<RedisWrapper>)
    ->Name("Redis/Insert")
    ->Range(1000, 100000)
    ->Unit(benchmark::kMillisecond);
...
// Point Lookup
BENCHMARK(BM_Lookup<MinilsmWrapper>)
    ->Name("Minilsm/Lookup")
    ->Range(1000, 100000)
    ->Unit(benchmark::kNanosecond);
BENCHMARK(BM_Lookup<RocksDBWrapper>)
    ->Name("RocksDB/Lookup")
    ->Range(1000, 100000)
    ->Unit(benchmark::kNanosecond);
BENCHMARK(BM_Lookup<RedisWrapper>)
    ->Name("Redis/Lookup")
    ->Range(1000, 100000)
    ->Unit(benchmark::kNanosecond);

// Iterator Scan
BENCHMARK(BM_Iterate<MinilsmWrapper>)
    ->Name("Minilsm/Iterate")
    ->Range(1000, 100000)
    ->Unit(benchmark::kMillisecond);
BENCHMARK(BM_Iterate<RocksDBWrapper>)
    ->Name("RocksDB/Iterate")
    ->Range(1000, 100000)
    ->Unit(benchmark::kMillisecond);
BENCHMARK(BM_Iterate<RedisWrapper>)
    ->Name("Redis/Iterate")
    ->Range(1000, 100000)
    ->Unit(benchmark::kMillisecond);

// Delete
BENCHMARK(BM_Delete<MinilsmWrapper>)
    ->Name("Minilsm/Delete")
    ->Range(1000, 100000)
    ->Unit(benchmark::kMillisecond);
BENCHMARK(BM_Delete<RocksDBWrapper>)
    ->Name("RocksDB/Delete")
    ->Range(1000, 100000)
    ->Unit(benchmark::kMillisecond);
BENCHMARK(BM_Delete<RedisWrapper>)
    ->Name("Redis/Delete")
    ->Range(1000, 100000)
    ->Unit(benchmark::kMillisecond);
