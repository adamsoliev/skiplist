...
struct BenchComparator
{
        ...
        DecodedType decode_key(const char *key) const { return key; }
};
...
class RocksDBWrapper
{
      public:
        ...
        RocksDBWrapper() : allocator_(), list_(BenchComparator(), &allocator_, 12, 4) {} // height=12, branch=4
        ...
        size_t memory_usage() const { return allocator_.MemoryUsage(); }

        class Iterator
        {
              public:
                explicit Iterator(const RocksDBWrapper *wrapper) : iter_(&wrapper->list_) {}
