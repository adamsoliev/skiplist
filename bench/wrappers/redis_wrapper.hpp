...
class RedisWrapper {
 public:
  ...
  class Iterator {
   public:
    explicit Iterator(const RedisWrapper* wrapper)
        : zsl_(wrapper->zsl_), node_(nullptr) {}
    ...
    void seek_to_first() {
      node_ = zslFirstNode(zsl_);
    }

    const char* key_data() const {
      if (!node_ || !node_->ele) return nullptr;
      return node_->ele->data + 4;  // Skip key length prefix
    }

    size_t key_size() const {
      if (!node_ || !node_->ele) return 0;
      uint32_t len;
