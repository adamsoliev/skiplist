...
class MinilsmWrapper
{
      public:
        ...
        size_t memory_usage() const { return arena_.memory_usage(); }

        class Iterator
        {
              public:
                explicit Iterator(const MinilsmWrapper *wrapper) : iter_(&wrapper->list_) {}
                ...
                const char *key_data() const { return iter_.key().user_key.data(); }

                size_t key_size() const { return iter_.key().user_key.size(); }
