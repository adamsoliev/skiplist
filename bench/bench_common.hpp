...
class TestData {
 public:
  TestData(size_t count, const DataProfile& profile, uint64_t seed = 42)
      : rng_(seed), count_(count) {
    keys_.reserve(count);
    values_.reserve(count);

    std::uniform_int_distribution<size_t> key_dist(
        profile.min_key_size, profile.max_key_size);
    std::uniform_int_distribution<size_t> val_dist(
        profile.min_value_size, profile.max_value_size);
