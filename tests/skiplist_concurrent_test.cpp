...
TEST_F(SkipListConcurrentTest, ConcurrentWritersDifferentKeys)
{
        ...
        for (int t = 0; t < kNumThreads; t++) {
                threads.emplace_back([this, t, &seq_counter]() {
...
TEST_F(SkipListConcurrentTest, ConcurrentReaders)
{
        ...
        std::vector<std::thread> threads;
        for (int t = 0; t < kNumThreads; t++) {
                threads.emplace_back([this, &successful_reads, &failed_reads]() {
...
TEST_F(SkipListConcurrentTest, ConcurrentIterators)
{
        ...
        std::vector<std::thread> threads;
        for (int t = 0; t < kNumThreads; t++) {
                threads.emplace_back([this, &all_passed]() {
                        // Forward iteration
                        SkipList::Iterator iter(list_.get());
                        int count = 0;
                        int prev_num = -1;
...
TEST_F(SkipListConcurrentTest, ReadersAndWritersMixed)
{
        ...
        for (int w = 0; w < kNumWriters; w++) {
                writers.emplace_back([this, w, &seq_counter]() {
...
TEST_F(SkipListConcurrentTest, IteratorsWhileWriting)
{
        ...
        std::thread writer([this, &seq_counter, &stop_writing]() {
                int i = 0;
        ...
        std::vector<std::thread> readers;
        for (int r = 0; r < 4; r++) {
                readers.emplace_back([this, &iter_count]() {
                        for (int round = 0; round < 10; round++) {
                                ...
                                for (iter.seek_to_first(); iter.valid(); iter.next()) {
                                        ...
                                        if (!prev_key.empty()) {
                                                EXPECT_LE(prev_key, key) << "Order violation: " << prev_key << " > " << key;
                                        }
                                        prev_key = key;
                                        count++;
                                }
...
TEST_F(SkipListConcurrentTest, RapidFireInsertAndRead)
{
        ...
        std::vector<std::thread> writers;
        for (int w = 0; w < 2; w++) {
                writers.emplace_back([this, &seq_counter, &stop, &total_inserts]() {
                        int local_inserts = 0;
        ...
        std::vector<std::thread> readers;
        for (int r = 0; r < 4; r++) {
                readers.emplace_back([this, &stop, &total_reads, &successful_reads]() {
                        std::mt19937 rng(std::random_device{}());
...
TEST_F(SkipListConcurrentTest, SequenceOrderingUnderContention)
{
        ...
        std::vector<std::thread> threads;
        for (int t = 0; t < kNumThreads; t++) {
                threads.emplace_back([this, &seq_counter]() {
                        for (int round = 0; round < kUpdatesPerKey; round++) {
                                for (int k = 0; k < kNumKeys; k++) {
                                        std::string key = "key" + std::to_string(k);
                                        uint64_t seq = seq_counter.fetch_add(1, std::memory_order_relaxed) + 1;
